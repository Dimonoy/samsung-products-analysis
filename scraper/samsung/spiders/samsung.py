...
class SamsungSpider(scrapy.Spider):
    ...
    async def parse(self, response: scrapy.http.Response):
        ...
        for card_url_locator in await page.locator("li.item a.link-review").all():
        ...
        for n, card_url in enumerate(cards_urls, start=1):
            context_number = n % amount_of_sets
            yield scrapy.Request(
                url=card_url,
                meta={ "playwright": True, "playwright_context": f"product_items_{context_number}" },
